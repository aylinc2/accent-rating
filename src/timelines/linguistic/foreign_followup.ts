...
export function createForeignFollowupTimeline(
  ...
  const selectForeignTrials = (): any[] => {
    ...
    const ratingOf = (trial: any) =>
      Number(trial.response?.accent_rating);

    const getHighestRandom = (trials: any[]) => {
      if (trials.length === 0) {
        return null;
      }

      const highestRating = Math.max(
        ...trials.map((trial: any) => ratingOf(trial))
      );

      const tiedHighest = trials.filter(
        (trial: any) => ratingOf(trial) === highestRating
      );

      return shuffle(tiedHighest)[0];
    };

    const highRatedForeignTrials = foreignTrials.filter((trial: any) => {
      const rating = ratingOf(trial);

      return Number.isFinite(rating) && rating >= 4 && rating <= 6;
    });

    const l2Trials = highRatedForeignTrials.filter(
      (trial: any) => trial.speaker_group === "L2"
    );

    const targetGroupTrials = highRatedForeignTrials.filter(
      (trial: any) =>
        isTurkish()
          ? trial.speaker_group === "HS"
          : trial.speaker_group === "Ret"
    );

    if (l2Trials.length > 0 && targetGroupTrials.length > 0) {
      const highestL2 = getHighestRandom(l2Trials);
      const highestTarget = getHighestRandom(targetGroupTrials);

      selectedTrials = [
        highestL2,
        highestTarget,
      ].filter((trial): trial is any => trial !== null);
